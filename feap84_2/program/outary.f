...
c       1. Add options to print arrays in correct size      22/12/2006
c       2. Add IEDOF to formated output list                25/09/2007
c       3. Change dimension check on 'LD' and 'P' to nst    05/10/2007
c       4. Reset 'narry' for XL                             24/11/2008
c       5. Mask S & P arrays to print only non-zero block   01/02/2012
c       6. Add formatted output for KTDIV                   16/05/2012
c       7. Change 'mr' to 'hr' form NORMV check             15/02/2013
c       8. Change 'ckinon0' to 'cknon0' for NORMV check     11/03/2013
c-----[--.----+----.----+----.-----------------------------------------]
...
c           Output formatted array values
...
c             Output global nodal coordinate array
...
c             Output nodal FPRO or ID array
...
c             Output global element IX connection and control array
...
c             Output element LD element assembly array
...
c             Output local nodal solution UL array
...
c             Output local element coordinate XL array
...
c             Output nodal NORMV normal vector array

              elseif(pcomp(array,'NORMV',5)) then
                if(cknon0(hr(point),3*numnp)) then
...
c             Output KTDIV array for nurbs

              elseif(pcomp(array,'KTDIV',5)) then
                if(ckinon0(mr(point),iupr)) then
                  call iprint(mr(point),4,iupr/4,4,dict(i))
                else
                  write(iow,2001) dict(i)
                    if(ior.lt.0) then
                      write(*,2001) dict(i)
...
c           Output other array values in unformatted form
...
c           Write remainder of array if necessary
...
c     Purpose: Find last non-zero entry in array

      implicit   none
      integer    nn,n
      real*8     v(nn)

c     Find non-zero entry
