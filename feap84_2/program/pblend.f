...
c       1. Call plstop on block type error,                 21/01/2007
c          add option to specify element type by text data.
c       2. Set iblend(7,numb) to 'ntyp' instead of into     03/05/2007
c          iblend(6,numb).
c       3. For btyp=2 store ntyp in iblend(7,numb), else    21/08/2007
c          store in iblend(6,numb).
c       4. Add 'mate' to set of material number             30/11/2010
c       5. Set 'btyp' as integer type; revise 'line' sets   16/04/2011
c          into 'iblend(19,*)'; remove dead code after call
c          to 'pblktyp'
c       6. Set iblend(6,numb) to type form pblktyp          18/07/2011
c       7. Add set of last element number to last_elm       29/01/2012
c       8. Correct set of ntyp after call to pblktyp        30/03/2012
c-----[--.----+----.----+----.-----------------------------------------]
c      Purpose: Generate nodes and elements for 1, 2 & 3-d problems
...
c     Save blend type for block

      iblend(19,numbl) = btyp

c     Set blend generation increments and first node/element/mate values
...
c     Record was vertex nodes of blend block
...
c     Determine last element number to be generated
      ...
      if(btyp.eq.1 .or. btyp.eq.4) then
        ni   = iblend(3,numbl)
        ne   = iblend(4,numbl)
        ma   = iblend(5,numbl)
        ntyp = iblend(6,numbl)
...
c     Set correct values into the iblend array
      ...
      if(btyp.eq.1 .or. btyp.eq.3 .or. btyp.eq.4) then
        iblend(3,numbl) = ni
        iblend(4,numbl) = ne
        iblend(5,numbl) = ma
        iblend(6,numbl) = ntyp
