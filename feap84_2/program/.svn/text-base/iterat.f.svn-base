...
c       1. add g*g0 < 0 for line search                     29/12/2008
c          add 'ct' to argument list
c       2. Modify exit strategy                             21/01/2009
c       3. Remove 'ct' from argument list                   02/05/2012
c-----[--.----+----.----+----.-----------------------------------------]
...
c-----[--.----+----.----+----.-----------------------------------------]
      ...
      logical   accrcy,prt
      integer   i,nupd,nbfgs, id(*)
      real*8    g,g0,s, onorm,dnorm,stol,etol, energyold
      real*8    oldrsd(*),d(*),v(*),w(*),t(*)
...
c       Compute step size using scalar line search and update solution
        ...
        if(abs(g).gt.stol*abs(g0) .and. g*g0 .le. 0.0d0) then
