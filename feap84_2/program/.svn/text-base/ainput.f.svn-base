c$Id:$
      logical function ainput(xxx,nc,d,nn)
...
c       Original version                                    25/02/2012
c-----[--.----+----.----+----.-----------------------------------------]
c      Purpose: Compute array of real values from string inputs

c      N.B.  Data items must fit within field widths of 25.
...
c     Check for upper case letters and convert to lower case
...
c     Format separators are blanks, commas, or equals
      ...
      if(cksep(xxx(n:n))) then
        if(n.gt.no) call asetval(xxx(no:n-1),n-no,d(nv))
31      n  = n  + 1
...
c     Fill in last value if needed
      ...
      if(n.gt.no.and.nv.le.nn) call asetval(xxx(no:n-1),n-no,d(nv))
