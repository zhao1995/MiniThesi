...
c     2. Delete erroneous 'pola' adjustments                05/03/2007
c     3. Add activation of generated node 7 for triangle;   06/03/2007
c        Correct placement of node counter for block;
c        Revise computation of x(k,*) to k = 1,ndm.
c-----[--.----+----.----+----.-----------------------------------------]
...
c      Outputs:
c         n         - Number last node on block (added by 7-node tris).
...
c-----[--.----+----.----+----.-----------------------------------------]
      ...
      integer    i,j,k,n,me,nq,nm,nn,n8,inc,ix(nen1,*),ilr(*)
      integer    ic(16),icr(16),iq(9),it(6)
      real*8     x(ndm,*)
            ...
            do i = 1,nr-1,inc
              ...
              elseif(abs(ntyp).eq.7) then
                ...
                if(ntyp.eq.-7) then
                  ...
                  ix(7,me) = n
                  do k = 1,ndm
                    x(k,n)   = (x(k,ix(it(4),me))
     &                       +  x(k,ix(it(5),me))
     &                       +  x(k,ix(it(6),me)))*four9
     &                       - (x(k,ix(it(1),me))
     &                       +  x(k,ix(it(2),me))
     &                       +  x(k,ix(it(3),me)))*one9
                  end do ! k
