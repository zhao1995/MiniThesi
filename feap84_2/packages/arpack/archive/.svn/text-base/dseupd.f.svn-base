c\BeginDoc
c-----[--.----+----.----+----.-----------------------------------------]
c     Modification log for FEAP interface             Date (dd/mm/year)
c       1. Allow for computation of all eigenpairs          07/09/2007
c       2. Introduce vdum(1) for passing arguments          02/07/2014
c-----[--.----+----.----+----.-----------------------------------------]
...
c        %-----------------------------------------------------------%
     ...
     &            'dseupd: Number of specified eigenvalues')
             call ivout(logfil, 1, nconv+npp, ndigit,
     &            'dseupd: Number of "converged" eigenvalues')
         end if

         if (numcnv .ne. nconv+npp) then
            info = -17
            go to 9000
         end if
...
c        %----------------------------------------%
         ...
         call dcopy (nconv+npp, workl(ihd), 1, d, 1)
...
c        %-----------------------------------------------------%
         ...
         call dcopy (nconv+npp, workl(ritz), 1, d, 1)
...
c        %---------------------------------------------------------%
         ...
         if (rvec) then
            call dsesrt ('LA', rvec , nconv+npp, d, ncv, workl(iq), ldq)
         else
...
c        %-------------------------------------------------------------%
         ...
         call dcopy (nconv+npp, workl(ihd), 1, d, 1)
         call dsortr ('LA', .true., nconv+npp, workl(ihd), workl(iw))
         if (rvec) then
            call dsesrt ('LA', rvec , nconv+npp, d, ncv, workl(iq), ldq)
         else
            call dcopy (ncv, workl(bounds), 1, workl(ihb), 1)
            call dscal (ncv, bnorm2/rnorm, workl(ihb), 1)
            call dsortr ('LA', .true., nconv+npp, d, workl(ihb))
         end if
...
c        %----------------------------------------------------------%
         ...
         call dgeqr2 (ncv, nconv+npp        , workl(iq) ,
...
c        %--------------------------------------------------------%
         ...
         call dorm2r ('R' , 'N'          , n        ,
     &                ncv , nconv+npp        , workl(iq),
     &                ldq , workl(iw+ncv), v        ,
     &                ldv , workd(n+1)   , ierr)
         call dlacpy ('A', n, nconv+npp, v, ldv, z, ldz)
...
c        %-------------------------------------------------%
     ...
     &                        / workl(iw+k-1)*(workl(iw+k-1)-one) )
      ...
      if (type .ne. 'REGULR' .and. msglvl .gt. 1) then
         call dvout (logfil, nconv+npp, d, ndigit,
     &          'dseupd: Untransformed converged Ritz values')
         call dvout (logfil, nconv+npp, workl(ihb), ndigit,
...
c     %-------------------------------------------------%
     ...
     &                  / (workl(iw+k)-one)
      ...
      if (type .ne. 'REGULR')
     &   call dger  (n, nconv+npp, one, resid, 1, workl(iw), 1, z, ldz)
