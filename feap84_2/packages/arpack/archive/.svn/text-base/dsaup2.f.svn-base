...
c     %---------------%
     ...
     &           np0, nptemp, nevd2, nevm2, kp(3)
      Double precision
     &           rnorm, temp, eps23
      real       tary(2), etime, t0,t1,t2,t3
      save       cnorm, getv0, initv, update, ushift,
     &           iter, kplusp, msglvl, nconv, nev0, np0,
     &           rnorm, eps23, t0,t1,t2,t3
...
c     %--------------------------------------------------------------%
 ...
 1000 continue
     ...
     &           'dsaup2: **** Start of major iteration number ****')
         end if
         if (msglvl .gt. 1) then
            idum(1) = nev
            call ivout (logfil, 1, idum, ndigit,
     &     'dsaup2: The length of the current Lanczos factorization')
            idum(1) = np
            call ivout (logfil, 1, idum, ndigit,
     &           'dsaup2: Extend the Lanczos factorization by')
         end if
...
c           %-----------------------------------------------------%
     ...
     &           'dsaup2: Current B-norm of residual for factorization')
         end if
...
c        %-------------------%
     ...
     &                  'dsaup2: NEV, NP, NCONV are')
            call dvout (logfil, kplusp, ritz, ndigit,
     &           'dsaup2: The eigenvalues of H')
            call dvout (logfil, kplusp, bounds, ndigit,
     &          'dsaup2: Ritz estimates of the current NCV Ritz values')
         end if
...
c           %------------------------------------------%
            ...
            if (msglvl .gt. 1) then
               call dvout (logfil, kplusp, ritz, ndigit,
     &            'dsaup2: Sorted Ritz values.')
               call dvout (logfil, kplusp, bounds, ndigit,
     &            'dsaup2: Sorted ritz estimates.')
            end if
...
c           %---------------------------------------%
     ...
     &              workl)
         ...
         if (msglvl .gt. 0) then
            idum(1) = nconv
            call ivout (logfil, 1, idum, ndigit,
     &           'dsaup2: no. of "converged" Ritz values at this iter.')
            if (msglvl .gt. 1) then
               kp(1) = nev
               kp(2) = np
               call ivout (logfil, 2, kp, ndigit,
     &              'dsaup2: NEV and NP are')
               call dvout (logfil, nev, ritz(np+1), ndigit,
     &              'dsaup2: "wanted" Ritz values.')
               call dvout (logfil, nev, bounds(np+1), ndigit,
     &              'dsaup2: Ritz estimates of the "wanted" values ')
            end if
         end if
...
c        %---------------------------------------------------------%
         ...
         if (msglvl .gt. 2) then
            idum(1) = np
            call ivout (logfil, 1, idum, ndigit,
     &                  'dsaup2: The number of shifts to apply ')
            call dvout (logfil, np, workl, ndigit,
     &                  'dsaup2: shifts selected')
            if (ishift .eq. 1) then
               call dvout (logfil, np, bounds, ndigit,
     &                  'dsaup2: corresponding Ritz estimates')
             end if
         end if
...
c           call second (t3)
         ...
         if (msglvl .gt. 2) then
            dum(1) = rnorm
            call dvout (logfil, 1, dum, ndigit,
     &      'dsaup2: B-norm of residual for NEV factorization')
            call dvout (logfil, nev, h(1,2), ndigit,
     &           'dsaup2: main diagonal of compressed H matrix')
            call dvout (logfil, nev-1, h(2,1), ndigit,
     &           'dsaup2: subdiagonal of compressed H matrix')
         end if
