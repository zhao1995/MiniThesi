...
c       1. Use idum(1) & dum(1) on calls: remove arg from   11/04/2013
c          call to pdgetv0.
c-----------------------------------------------------------------------
c\BeginDoc
...
c     %---------------------------------------------%
      ...
      if (getv0) then
         call pdgetv0 (ido, bmat, initv, n, 1, v, ldv, resid, rnorm,
     &                 ipntr, workd, info)
...
c           %-----------------------------------------------------%
            ...
            np     = info
            mxiter = iter
            if(info.eq.nummasst) then ! Have all values
              info =  0
            else
              info = -9999
            endif
