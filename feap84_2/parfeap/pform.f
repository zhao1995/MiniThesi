...
c      13. Add call to pparlo and set nar for reactions     13/12/2013
c-----[--.----+----.----+----.-----------------------------------------]
...
c             Assemble arrays as necessary
     ...
     &                           mr(np(31)+ndf*numnp),nst,nen)
                  ...
                  elseif (jsw.ne.8 .and. np(245).ne.0) then
                    call pparlo(ld,mr(np(245)),ix(1,n),
     &                           mr(np(31)+ndf*numnp),nst,nen)
                    call pmodify(ld,p,s,p(1,3),nst,nst)
                  endif
                  if(jsw.eq.5) then               ! Mass assembly
                    call uasblem(s,p,ld,nst,aufl,bfl)
                  else                            ! Other assembly
