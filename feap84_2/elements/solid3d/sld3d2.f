...
c       3. Set range to 3 for computing x0(*)               03/01/2007
c       4. Revise form of statement for augmenting          26/01/2007
c       5. Change '10' to '27' on call to slcn3d            05/03/2007
c       6. Add augmenting for all element forms;            14/03/2007
c          Use d(185) for augmenting factor;
c          Change epsm(4) to epsm(6) for d(78) use;
c          Correct format 2002 for stress outputs
c       7. Correct computation of 4-node tet intertia.      29/03/2007
c       8. Add 14/15 node tet computation                   06/09/2007
c       9. Set number element multipliers for 14/15 tets    07/09/2007
c      10. Introduce 'nvn' to compute center from vertex    14/09/2007
c      11. Change 'ord' to 'nel' on 'tetshp' calls          05/11/2007
c      12. Modify quadrature for interia, add 'iner3d'      28/12/2007
c      13. Add output of principal strains                  04/03/2008
c      14. Add direct call to quadr3d and interp3d          11/11/2008
c      15. Remove 'nel' from call to 'quadr3d'              23/01/2009
c      16. Increase arrays to store 64 node brick           03/02/2009
c          Add quadratic terms to phi array.
c      17. Revise augment: augfp -> augf; nhi -> fp(1)      14/04/2009
c      18. Introduce tdof to correct thermal tangent        30/04/2009
c      19. Add 'elcoor' to use xref                         25/07/2009
c      20. Move computation of psil to material models      26/07/2009
c      21. Set augment 'npm' by call to quadr3d             15/08/2009
c      22. Increase arrays to store 125 node brick          20/12/2010
...
c      25. Delete extra call to iner3d.f                    13/04/2011
c      26. Add 'l' to modlsd call                           05/01/2012
...
c-----[--.----+----.----+----.-----------------------------------------]
      ...
      real*8    r(ndf,*) ,  xx(3,125)
      real*8    bbd(3,7) ,  aa(6,6,5,125),  dd(7,7)
      real*8    sigm(9)  ,  sigl(10,125) ,  x0(3)    ,   bbar(3,125,125)
      real*8    phi(10,125), theta(3,125),  hh(10,10),   epsl(6,125)
      real*8    press(125),  pbar(125)   ,  hsig(10) ,   eps(9,3,125)
      real*8    ta(125)  ,  epsm(9)      ,  epsv(125),   mom(3,125)
      real*8    dther(7) ,  body(3)      ,  bf(3)    ,   btan(3,3)
      real*8    rr(10)   ,  vl(3)        ,  bt(3)
...
c                   Thermo-mechanical coupling matrix
                    ...
                    if(ther) then
                      do jj = 1,3
                      s(i1+jj,j1+tdof)  = s(i1+jj,j1+tdof)
     &                                  + bt(jj)*shp3(4,j,l)
                      end do ! jj
                    endif
...
c           Inertial contributions to residual and tangent
...
c       Compute J-integrals and material forces
        ...
        elseif(isw.eq.16) then
          ...
          call pjint3d(ul,tl,epsv,sigl,r,ndf,ndm)
