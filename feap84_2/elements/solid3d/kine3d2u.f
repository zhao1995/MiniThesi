c$Id:$
      subroutine kine3d2u(shp,dvol0,ul,f,fi,df,fdet,detf,theta,shp0,
     &                    shp1,shptil,V0,ndf,nel,nen,lint)
...
c         shptil(3,8)   - Averaged theta   shape functions
c-----[--.----+----.----+----.-----------------------------------------]
      implicit  none
      ...
      real*8    detfi, V0, V0i, ratio1,ratio2
      real*8    shp(4,125,*),dvol0(*),ul(ndf,nen,*)
      real*8    df(3,3),f(3,3,2),dfl(3,3),fl(3,3,2),fi(3,3)
      real*8    shp0(3,8), shp1(3,8), shptil(3,8),fdet(*),detf(2)
      real*8    theta(2)
...
c       i = 1: det F at t_n+1; i = 2: det F at t_n
     ...
     &            + fl(1,3,i)*(fl(2,1,i)*fl(3,2,i)-fl(3,1,i)*fl(2,2,i))
        ...
        do i = 1,2
          theta(i) = theta(i) + detf(i)*dvol0(l)
        end do ! i
...
c       Compute integral of current shape functions

        detfi = detf(1)*dvol0(l)
        do i = 1,8
          do j = 1,3
            shptil(j,i) = shptil(j,i) + shp1(j,i)*detfi
          end do ! j
        end do ! i
...
c     Compute modified deformation gradient (G - in Bonet & Bhargava)

      ratio1 = abs(theta(1)/detf(1))**one3
      ratio2 = abs(theta(2)/detf(2))**one3
      do j = 1,3
        do i = 1,3
          f(i,j,1) = ratio1*f(i,j,1)        ! G_n+1
          f(i,j,2) = ratio2*f(i,j,2)        ! G_n
        end do ! i
      end do ! l
