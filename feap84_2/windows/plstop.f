...
c          Add parallel stop call.
c       2. Set prob_on to true after call to ptimpl         15/09/2009
c          Move pdelfl after tplot options.
c-----[--.----+----.----+----.-----------------------------------------]
...
c     Do last tplot outputs if required
      ...
      if(.not.prob_on) then
        rfl = .false.
        call ptimpl()
        prob_on = .true.   ! Prevent time plot from causing recurrsion
      endif
